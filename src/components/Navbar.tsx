...
const Navbar = () => {
  ...
  const categories = [
    ...
    { name: 'Men\'s Collection', href: '#mens-collection' },
      ...
      <nav className="max-w-7xl mx-auto flex items-center justify-between">
        <a href="#" className="flex items-center">
          <span className={cn(
            ...
            isScrolled ? 'text-neon-pink' : 'text-white'
          )}>UK Store</span>
        </a>
        ...
        <div className="hidden md:flex space-x-8">
          {categories.map((category) => (
            <a
              ...
              href={category.href}
              className={cn(
                'text-sm font-medium transition-colors duration-300 hover:text-neon-pink',
          ...
          <div className="px-6 py-4 space-y-2">
            {categories.map((category) => (
              <a
                key={category.name}
                href={category.href}
                className="block py-2 text-foreground hover:text-neon-pink"
